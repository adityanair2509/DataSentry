...
class Dashboard:
    ...
    def _get_geographic_data(self):
        ...
        try:
            ...
            # Only process actual IP addresses, not domain names
            cursor.execute('''
                SELECT 
                ...
                FROM dns_queries 
                WHERE created_at > datetime('now', '-24 hours')
                AND dest_ip NOT LIKE '%.com'
                AND dest_ip NOT LIKE '%.org'
                AND dest_ip NOT LIKE '%.net'
                AND dest_ip GLOB '[0-9]*.[0-9]*.[0-9]*.[0-9]*'
