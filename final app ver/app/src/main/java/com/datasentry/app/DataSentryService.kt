...
class DataSentryService : VpnService() {
    ...
    // Analytics client for server communication (encrypted)
    private lateinit var analyticsClient: SecureAnalyticsClient
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        analyticsClient = SecureAnalyticsClient(this)
        
        // Clear old encrypted logs for fresh start (debugging)
        serviceScope.launch {
            try {
                analyticsClient.clearOldLogs()
            } catch (e: Exception) {
                Log.e("DataSentryService", "Failed to clear old logs: ${e.message}")
            }
        }
        ...
        try {
            ...
            if (REAL_MODE) {
                ...
                vpnInterface?.let { pfd ->
                    ...
                    dnsHandler = DnsOnlyHandler(this, pfd, packetRepository, analyticsClient)
                    dnsHandler?.start()
