...
import com.datasentry.app.security.AnalysisManager
...
class DnsOnlyHandler(
    ...
    private val analyticsClient: SecureAnalyticsClient
) {
    companion object {
        private const val TAG = "DnsOnlyHandler"
        private const val BUFFER_SIZE = 4096
        private const val DNS_PORT = 53
        private const val DNS_SERVER_1 = "8.8.8.8" // Google DNS
    }
    
    // Shared AnalysisManager instance
    private val analysisManager: AnalysisManager by lazy {
        AnalysisManager(vpnService.applicationContext)
    }
    ...
    private fun runDnsLoop() {
        ...
        try {
            ...
            while (isRunning.get()) {
                ...
                val currentPacketCount = packetCount.incrementAndGet()
                
                // Parse IP packet
                val parsed = PacketParser.parse(buffer, bytesRead)
                if (parsed == null) {
                    Log.w(TAG, "Failed to parse packet #$currentPacketCount")
                    continue
                }
                
                Log.d(TAG, "Packet #$currentPacketCount: ${parsed.protocolName} ${parsed.sourcePort} -> ${parsed.destIp}:${parsed.destPort}")
                
                // Check if this is a DNS query (UDP port 53)
                if (parsed.protocol == 17 && parsed.destPort == 53) {
                    Log.d(TAG, "DNS packet detected: #$currentPacketCount")
                    handleDnsQuery(buffer, parsed)
                }
            }
        } catch (e: Exception) {
     ...
     */
    private fun handleDnsQuery(buffer: ByteArray, parsed: PacketParser.ParsedPacket) {
        try {
            ...
            Log.d(TAG, "DNS Query parsed: domain=$domain, payloadSize=${dnsPayload.size}")
            
            if (domain != null) {
                // Log only once per second to avoid spam
                val now = System.currentTimeMillis()
                val currentPacketCount = packetCount.get()
                if (now - lastLogTime > 1000) {
                    Log.d(TAG, "DNS Query: $domain (total packets: $currentPacketCount)")
                    lastLogTime = now
                }
                
                // Log to database
                Log.d(TAG, "Calling logDnsQuery with domain: $domain")
            ...
            try {
                ...
                val responseIpPacket = buildDnsResponsePacket(
                    originalPacket = buffer.copyOf(parsed.totalLength),
                    parsed = parsed,
    ...
    private fun parseDnsDomain(dnsPayload: ByteArray): String? {
        if (dnsPayload.size < 12) {
            Log.d(TAG, "DNS payload too small: ${dnsPayload.size}")
            return null
        }
        ...
        try {
            ...
            val result = if (domain.isNotEmpty()) domain.toString() else null
            Log.d(TAG, "Parsed DNS domain: $result (payload size: ${dnsPayload.size})")
            return result
        } catch (e: Exception) {
    ...
    private fun logDnsQuery(domain: String, parsed: PacketParser.ParsedPacket) {
        Log.d(TAG, "logDnsQuery called with domain: $domain")
        scope.launch {
            try {
                val (appName, contentType) = detectAppFromDomain(domain)
                Log.d(TAG, "Detected app: $appName, content: $contentType")
                ...
                Log.d(TAG, "Creating PacketEntity with domain: $domain, app: $appName")
                packetRepository.logPacket(packet)
                
                // Send to SecureAnalyticsClient for encrypted storage and server upload
                Log.d(TAG, "Sending to SecureAnalyticsClient: $domain")
                
                // Analyze packet using current analysis mode
                Log.d(TAG, "Analyzing packet with AnalysisManager: $domain")
                val analyzedPacket = analysisManager.analyzePacket(packet)
                
                // Update packet in database with analysis results
                packetRepository.updatePacket(analyzedPacket)
                
                // Send analyzed packet to SecureAnalyticsClient for server upload
                val result = analyticsClient.sendEncryptedData(listOf(analyzedPacket))
                Log.d(TAG, "Sent analyzed packet to server: $domain, result: $result")
