...
import com.datasentry.app.network.AnalyticsClient
...
class DnsOnlyHandler(
    ...
    private fun logDnsQuery(domain: String, parsed: PacketParser.ParsedPacket) {
        ...
        scope.launch {
            try {
                // Use TrafficStats to find the real app that made this request
                val activeApp = trafficStatsHelper.findMostActiveApp(domain)
                
                val (appName, packageName) = if (activeApp != null) {
                    Log.d(TAG, "TrafficStats found: ${activeApp.appName} (${activeApp.packageName})")
                    activeApp.appName to activeApp.packageName
                } else {
                    // Fallback to domain-based detection
                    val (name, _) = detectAppFromDomain(domain)
                    Log.d(TAG, "Fallback detection: $name")
                    name to ""
                }
                
                val (_, contentType) = detectAppFromDomain(domain)
                
                // Try to get real traffic stats
                ...
                Log.d(TAG, "PacketEntity: app=$appName, pkg=$packageName, domain=$domain")
                packetRepository.logPacket(packet)
                
                // Send to AnalyticsClient
                analyticsClient.addDnsQuery(packet)
